'use client';
import {
  AiOutlineShopping,
  AiOutlineSwap,
  AiOutlineCreditCard,
  AiOutlineCheckCircle,
  AiOutlineUndo
} from 'react-icons/ai';
import { MdStorefront, MdDeliveryDining } from 'react-icons/md';
import Image from 'next/image';
import { useState, useRef, useEffect } from 'react';

const CartPage: React.FC = () => {
  const [activeIndex, setActiveIndex] = useState<number>(0);
  const totalSlides = 5;
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleScroll = () => {
      if (scrollRef.current) {
        const scrollLeft = scrollRef.current.scrollLeft;
        const slideWidth = scrollRef.current.scrollWidth / totalSlides;
        setActiveIndex(Math.round(scrollLeft / slideWidth));
      }
    };

    const container = scrollRef.current;
    if (container) {
      container.addEventListener('scroll', handleScroll);
      return () => container.removeEventListener('scroll', handleScroll);
    }
  }, []);

  return (
    <div className='p-6 flex flex-col items-center text-center'>
      <AiOutlineShopping className='text-5xl mb-4' />
      <h2 className='text-xl font-bold mb-2'>Sepetiniz Boş</h2>
      <p className='mb-6 text-xs'>
        Alışverişe başlayın ve yeni gelen ürünlerimize göz atın.
      </p>

      <section className='grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-6'>
        <a
          href='#'
          className='text-xs font-bold border border-black px-6 py-2 hover:bg-black hover:text-white transition text-center'
        >
          Kadın
        </a>
        <a
          href='#'
          className='text-xs font-bold border border-black px-6 py-2 hover:bg-black hover:text-white transition text-center'
        >
          Erkek
        </a>
        <a
          href='#'
          className='text-xs font-bold border border-black px-6 py-2 hover:bg-black hover:text-white transition text-center'
        >
          Çocuk
        </a>
        <a
          href='#'
          className='text-xs font-bold border border-black px-6 py-2 hover:bg-black hover:text-white transition text-center'
        >
          Bebek
        </a>
      </section>

      <div className='mt-10 p-6 w-full flex items-center justify-between'>
        <div className='text-left'>
          <h3 className='text-lg font-bold text-black'>
            İLK SİPARİŞE ÖZEL TÜM ÜRÜNLERDE{' '}
            <span className='text-red-600'>KARGO BEDAVA</span> <br />+ YENİ
            SEZONDA <span className='text-red-600'>%10 İNDİRİM!</span>
          </h3>
          <p className='text-sm text-gray-600 mt-2'>
            Kampanyalar 28.02.2025 saat 23:59 kadar geçerlidir.
          </p>
        </div>

        <Image
          src='/images/kampanya-banner.png'
          alt='Kampanya Banner'
          width={200}
          height={100}
          className='rounded-lg'
        />
      </div>

      <section className='bg-white border-t border-gray-200 py-6 mt-10 w-full'>
        <div className='max-w-full mx-auto px-10 relative'>
          {/* Kaydırma Alanı */}
          <div
            ref={scrollRef}
            className='flex w-full overflow-x-auto  flex-nowrap hide-scrollbar md:hide-scrollbar '
          >
            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <MdStorefront className='text-3xl mb-2' />
              <p className='text-xs'>Digital Emporium</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <MdStorefront className='text-3xl mb-2' />
              <p className='text-xs'>Mağazadan Gel-Al</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <AiOutlineSwap className='text-3xl mb-2' />
              <p className='text-xs'>Mağazada Değişim & İade</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <AiOutlineCreditCard className='text-3xl mb-2' />
              <p className='text-xs'>Kapıda Ödeme</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <MdDeliveryDining className='text-3xl mb-2' />
              <p className='text-xs'>Bi Tıkla Kapında</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <AiOutlineCheckCircle className='text-3xl mb-2' />
              <p className='text-xs'>Güvenli Alışveriş</p>
            </div>
            <div className='h-10 w-px bg-black hidden lg:block'></div>

            <div className='flex flex-1 flex-col items-center px-4 text-center font-bold min-w-[120px]'>
              <AiOutlineUndo className='text-3xl mb-2' />
              <p className='text-xs'>Ücretsiz İade</p>
            </div>
          </div>

          {/* Sayfa Noktaları */}
          <div className='flex justify-center items-center py-4 w-full md-lg:hidden lg:hidden mb-3'>
            {Array.from({ length: totalSlides }).map((_, index) => (
              <span
                key={index}
                className={`inline-block h-2.5 w-2.5 rounded-full mx-1 border border-[#000] ${
                  activeIndex === index ? 'bg-[#000] border-none' : 'bg-white'
                }`}
              />
            ))}
          </div>
        </div>
      </section>
    </div>
  );
};

export default CartPage;
